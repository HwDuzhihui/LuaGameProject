        ...
        switch (tran.transactionState) {
            ...
            case SKPaymentTransactionStateRestored:
                ...
                if (tran != nil && tran.payment != nil) {
                    ...
                    UnitySendMessage("IAP", "BuyRestore" ,[tran.payment.productIdentifier UTF8String] );
...
- (void)cancelTransaction{
	...
	UnitySendMessage("IAP", "CancelBuy", "");
}
...
void  SavePhoto(char *Address)
{
    NSString *ImagePatch = [NSString stringWithUTF8String:Address];
    UIImage *ima = [UIImage imageWithContentsOfFile:ImagePatch];
    
    if (payVC == nil) {
        payVC = [[PaymentViewController alloc]init];
    }
    if(ima == nil)
	{
	    return;
    ...
    UIImageWriteToSavedPhotosAlbum(ima, payVC, @selector(imageSavedCallBack:didFinishSavingWithError:contextInfo:),nil);
}


-(void) imageSavedCallBack:(UIImage *) image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if (error != nil) {
        NSLog(@"保存照片失败");
    }
